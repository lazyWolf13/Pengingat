...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Gambar Gallery</h1>
        ...
        <form action="{{ route('admin.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div class="border border-gray-300 rounded-lg p-4">
                    ...
                    <select name="post_id" id="post_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        ...
                        @foreach($posts as $post)
                            <option value="{{ $post->id }}" {{ old('post_id', $gallery->post_id) == $post->id ? 'selected' : '' }}>
                                {{ $post->judul }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="border border-gray-300 rounded-lg p-4">
                    ...
                    <input type="number" name="position" id="position" value="{{ old('position', $gallery->position) }}" 
