...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto bg-white p-8 rounded-lg shadow">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Gambar</h1>
        ...
        <form action="{{ route('admin.image.update', $image->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="gallery_id" class="w-full border-gray-300 rounded-lg">
                    ...
                    @foreach($galleries as $gallery)
                        <option value="{{ $gallery->id }}" {{ $image->gallery_id == $gallery->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="judul" value="{{ old('judul', $image->judul) }}" class="w-full border-gray-300 rounded-lg">
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 mb-2">File Gambar (Kosongkan jika tidak mengganti)</label>
                <input type="file" name="file" class="w-full border-gray-300 rounded-lg">

                <div class="mt-4">
                    <p class="text-gray-600 mb-2">Gambar Saat Ini:</p>
                    <img src="{{ asset('storage/' . $image->file) }}" alt="{{ $image->judul }}" class="h-32">
                </div>
            </div>
