    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.posts.update', $post) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul', $post->judul) }}" 
            ...
            <div class="space-y-2">
                ...
                <textarea id="isi" name="isi" rows="10" 
                    class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('isi') border-red-500 @enderror">{{ old('isi', $post->isi) }}</textarea>
                @error('isi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
