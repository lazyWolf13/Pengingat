...
<body class="bg-gradient-to-br from-green-100 via-green-200 to-green-300 min-h-screen">
    ...
    <div class="flex">
        <!-- Sidebar -->
        <aside id="sidebar"
            class="w-64 bg-gradient-to-t from-green-300 to-green-200 text-gray-900 p-6 fixed top-0 left-0 h-screen shadow-lg overflow-y-auto transition-transform transform">
            <!-- Tombol close -->
            <button id="closeSidebar"
                class="absolute top-4 right-4 text-gray-700 hover:text-red-500 text-2xl">&times;</button>
            ...
            <div class="flex items-center space-x-4 mb-6 pb-6 border-b border-green-400">
                <div class="h-12 w-12 rounded-full bg-white flex items-center justify-center">
                    <span
                        class="text-xl font-bold text-green-600">{{ auth()->check() ? strtoupper(substr(auth()->user()->full_name, 0, 1)) : 'U' }}</span>
                </div>
                <div>
                    <h2 class="text-sm font-semibold text-gray-900">
                        {{ auth()->check() ? auth()->user()->full_name : 'User' }}</h2>
